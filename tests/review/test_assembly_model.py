...
def test_detectability_is_stored_flat_and_read_as_tuples():
    ...
    a = Assembled(detectability_flat={
        Assembled.key(ImageRole.FRONT, "top_left", "corners", "rounding"): "high"})
    assert a.detectability[(ImageRole.FRONT, "top_left", "corners", "rounding")] is Scale.HIGH


def test_the_model_round_trips_through_json_with_keys_intact():
    a = Assembled(
        detectability_flat={Assembled.key(ImageRole.FRONT, "top_left", "corners", "whitening"): "low"},
        reason_codes_flat={Assembled.key(ImageRole.FRONT, "top_left", "corners", "whitening"): "WHITE_BORDER"},
        faces_present=["front"])
...
def test_a_dump_of_the_model_canonicalizes_for_a_fingerprint():
    ...
    a = Assembled(detectability_flat={
        Assembled.key(ImageRole.BACK, "center", "surface", "scratches"): "moderate"})
